...
client.on('interactionCreate', async (interaction) => {
  ...
  if (commandName === 'add_token') {
    ...
    if (Object.values(config).some(entry => entry.userId === user.id)) {
      await interaction.reply({ content: 'You have already added a token.', ephemeral: true });
      return;
    }
    ...
    if (config[token] && config[token].userId === user.id) {
      ...
      await interaction.reply(`Token not found or you do not have permission to remove it.`);
    }
